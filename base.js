...
var app = new _vue2.default({
    ...
    methods: {
        updateTodos: function updateTodos() {
            ...
            var dataString = JSON.stringify(this.todos); // JSON 在序列化这个有 id 的数组的时候，会得出怎样的结果？
        ...
        saveTodos: function saveTodos() {
            ...
            avTodos.save().then(function (todo) {
                _this.todos.id = todo.id; // 一定要记得把 id 挂到 this.todos 上，否则下次就不会调用 updateTodos 了
                console.log('保存成功');
            }, function (error) {
                alert('保存失败');
            });
        ...
        doneEdit: function doneEdit(todo, index) {
            this.editedTodo = null;
            if (!todo.content) {
                this.removeTodo(index);
            }
        },
        cancleEdit: function cancleEdit(todo) {
            this.editedTodo = null;
        ...
        fetchTodos: function fetchTodos() {
            ...
            if (this.currentUser) {
                ...
                query.find().then(function (todos) {
                    ...
                    _this4.todos = JSON.parse(avAllTodos.attributes.content); // 为什么有个 attributes？因为我从控制台看到的
                    _this4.todos.id = id; // 为什么给 todos 这个数组设置 id？因为数组也是对象啊
                }, function (error) {
                    console.error(error);
                });
            }
